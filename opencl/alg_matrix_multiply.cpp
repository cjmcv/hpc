...
int main(int argc, char **argv) {
  ...
  size_t height_a = 32, width_a = 32;
  size_t height_b = 32, width_b = 18;
  if (width_a != height_b) {
  ...
  size_t local_work_size[2] = {8, 8}; // x, y
  ...
  for (cl_uint i = 0; i < num_platforms; i++) {
    ...
    loader->GetKernel("MatrixMulDevice", &kernel);
    ...
    */
    ...
    printf("Test: %s (%f, %f)\n \n", (meam_cl == mean ? "PASS" : "FAILED"), meam_cl, mean);
