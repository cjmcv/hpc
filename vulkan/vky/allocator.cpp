...
template<class T>
void Allocator<T>::to_host(T *out_data, int len) {
  if (flags_ & vk::MemoryPropertyFlagBits::eHostVisible) { // memory IS host visible
    ...
    auto stage_buf = Allocator(*dev_, physdev_, size(),
      vk::MemoryPropertyFlagBits::eHostVisible,
...
template<class T>
vk::BufferUsageFlags Allocator<T>::update_usage(const vk::PhysicalDevice& phys_device,
...
template<class T>
uint32_t Allocator<T>::SelectMemory(const vk::PhysicalDevice& phys_dev,
...
template<class T>
vk::DeviceMemory Allocator<T>::AllocMemory(const vk::Device& device,
                                           const vk::Buffer& buf,
...
template<class T>
vk::Buffer Allocator<T>::CreateBuffer(const vk::Device& device,
