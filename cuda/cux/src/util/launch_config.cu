...
Config1D LaunchConfig::GetKernelConfig1D(int element_count, const void *kernel,
  ...
  CUDA_CHECK(cudaOccupancyMaxPotentialBlockSize(
    ...
    &config.threads_per_block,   // Suggested block size to achieve maximum occupancy.
    kernel, 
  ...
  // Round up.
  config.blocks_per_grid = std::min(min_grid_size, DivUp(element_count, config.threads_per_block));
  return config;
}
...
Config3D LaunchConfig::GetKernelConfig3D(int xdim, int ydim, int zdim, const void *kernel,
  ...
  cudaError_t err = cudaOccupancyMaxPotentialBlockSize(
    &block_count, &threads_per_block, kernel, dynamic_shared_memory_size,
    block_size_limit);
  //CHECK_EQ(err, cudaSuccess);

  int threadsx = std::min({ xdim, threads_per_block, xthreadlimit });
  int threadsy =
    std::min({ ydim, std::max(threads_per_block / threadsx, 1), ythreadlimit });
  int threadsz =
    std::min({ zdim, std::max(threads_per_block / (threadsx * threadsy), 1),
      zthreadlimit });
  ...
  config.threads_per_block = dim3(threadsx, threadsy, threadsz);
  config.blocks_per_grid = dim3(blocksx, blocksy, blocksz);
  return config;
}
} // cux.
